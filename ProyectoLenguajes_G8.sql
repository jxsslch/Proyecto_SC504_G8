...
-- 1. Paquete para el CRUD de los doctores

CREATE OR REPLACE PACKAGE doctor_package AS
    PROCEDURE AgregarDoctor(
        p_cedDoctor NUMBER,
        p_puesto VARCHAR2,
        p_nomDoctor VARCHAR2,
        p_genero CHAR,
        p_idHorario NUMBER
    );

    PROCEDURE ActualizarDoctor(
        p_cedDoctor NUMBER,
        p_puesto VARCHAR2,
        p_nomDoctor VARCHAR2,
        p_genero CHAR,
    ...
    PROCEDURE EliminarDoctor(p_cedDoctor NUMBER);
    
    FUNCTION TraerDoctor(p_cedDoctor NUMBER) RETURN doctor%ROWTYPE;
END doctor_package;
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY doctor_package AS
    PROCEDURE AgregarDoctor(
        p_cedDoctor NUMBER,
        p_puesto VARCHAR2,
        p_nomDoctor VARCHAR2,
    ...
    BEGIN
        UPDATE doctor
        SET puesto = p_puesto,
            nomDoctor = p_nomDoctor,
            genero = p_genero,
            idHorario = p_idHorario
        WHERE cedDoctor = p_cedDoctor;
    END ActualizarDoctor;

    PROCEDURE EliminarDoctor(p_cedDoctor NUMBER) AS
    BEGIN
        DELETE FROM doctor WHERE cedDoctor = p_cedDoctor;
    END EliminarDoctor;
    
    FUNCTION TraerDoctor(p_cedDoctor NUMBER) RETURN doctor%ROWTYPE AS
...
-- 2. Paquete para el CRUD de los pacientes

CREATE OR REPLACE PACKAGE paciente_package AS
    PROCEDURE AgregarPaciente(
        p_cedPaciente NUMBER,
        p_nomPaciente VARCHAR2,
        p_fechaNacimiento DATE,
        p_edad NUMBER,
        p_genero CHAR
    );

    PROCEDURE ActualizarPaciente(
        p_cedPaciente NUMBER,
        p_nomPaciente VARCHAR2,
        p_fechaNacimiento DATE,
        p_edad NUMBER,
    ...
    PROCEDURE EliminarPaciente(p_cedPaciente NUMBER);
    
    FUNCTION TraerPaciente(p_cedPaciente NUMBER) RETURN paciente%ROWTYPE;
...
CREATE OR REPLACE PACKAGE BODY paciente_package AS
    ...
    BEGIN
        UPDATE paciente
        SET nomPaciente = p_nomPaciente,
            edad = p_edad,
            genero = p_genero
        WHERE cedPaciente = p_cedPaciente;
    END ActualizarPaciente;

    PROCEDURE EliminarPaciente(p_cedPaciente NUMBER) AS
    BEGIN
        DELETE FROM paciente WHERE cedPaciente = p_cedPaciente;
    END EliminarPaciente;
    
    FUNCTION TraerPaciente(p_cedPaciente NUMBER) RETURN paciente%ROWTYPE AS
...
-- 3. Paquete para el CRUD de los horarios

CREATE OR REPLACE PACKAGE horario_package AS
    PROCEDURE AgregarHorario(
        p_idHorario NUMBER,
        p_dias VARCHAR2,
        p_horas TIMESTAMP
    );
    ...
    PROCEDURE EliminarHorario(p_idHorario NUMBER);
    
    FUNCTION TraerHorario(p_idHorario NUMBER) RETURN horario%ROWTYPE;
END horario_package;
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY horario_package AS
    PROCEDURE AgregarHorario(
        p_idHorario NUMBER,
        p_dias VARCHAR2,
        p_horas TIMESTAMP
    ...
    PROCEDURE EliminarHorario(p_idHorario NUMBER) AS
    BEGIN
        DELETE FROM horario WHERE idHorario = p_idHorario;
    END EliminarHorario;
    
    FUNCTION TraerHorario(p_idHorario NUMBER) RETURN horario%ROWTYPE AS
...
-- 4. Paquete para el CRUD de medicamento_dosis

CREATE OR REPLACE PACKAGE medicamento_dosis_package AS
    PROCEDURE AgregarMedDosis(
        p_idDosisMedicamento NUMBER,
        p_idDosis NUMBER,
        p_idMedicamento NUMBER
    );

    PROCEDURE ActualizarMedDosis(
        p_idDosisMedicamento NUMBER,
        p_idDosis NUMBER,
        p_idMedicamento NUMBER
    );

    PROCEDURE EliminarMedDosis(p_idDosisMedicamento NUMBER);
    
    FUNCTION TraerMedDosis(p_idDosisMedicamento NUMBER) RETURN medicamento_dosis%ROWTYPE;
END medicamento_dosis_package;
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY medicamento_dosis_package AS
    PROCEDURE AgregarMedDosis(
        p_idDosisMedicamento NUMBER,
        p_idDosis NUMBER,
        p_idMedicamento NUMBER
    ...
    BEGIN
        UPDATE medicamento_dosis
        SET idDosis = p_idDosis,
            idMedicamento = p_idMedicamento
        WHERE idDosisMedicamento = p_idDosisMedicamento;
    END ActualizarMedDosis;

    PROCEDURE EliminarMedDosis(p_idDosisMedicamento NUMBER) AS
    BEGIN
        DELETE FROM medicamento_dosis WHERE idDosisMedicamento = p_idDosisMedicamento;
    END EliminarMedDosis;
    
    FUNCTION TraerMedDosis(p_idDosisMedicamento NUMBER) RETURN medicamento_dosis%ROWTYPE AS
        v_MedDosis medicamento_dosis%ROWTYPE;
    BEGIN
        SELECT *
        INTO v_MedDosis
...
-- 5. Paquete para el CRUD de paciente_nacionalidad

CREATE OR REPLACE PACKAGE paciente_nacionalidad_package AS
    PROCEDURE AgregarPacNac(
        p_idPacienteNacionalidad NUMBER,
        p_idNacionalidad NUMBER,
        p_cedPaciente NUMBER
    );

    PROCEDURE ActualizarPacNac(
        p_idPacienteNacionalidad NUMBER,
        p_idNacionalidad NUMBER,
        p_cedPaciente NUMBER
    );

    PROCEDURE EliminarPacNac(p_idPacienteNacionalidad NUMBER);
    
    FUNCTION TraerPacNac(p_idPacienteNacionalidad NUMBER) RETURN paciente_nacionalidad%ROWTYPE;
END paciente_nacionalidad_package;
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY paciente_nacionalidad_package AS
    PROCEDURE AgregarPacNac(
        p_idPacienteNacionalidad NUMBER,
        p_idNacionalidad NUMBER,
        p_cedPaciente NUMBER
    ...
    BEGIN
        INSERT INTO paciente_nacionalidad(
            idPacienteNacionalidad,
            idNacionalidad,
            cedPaciente
        ) VALUES (
            p_idPacienteNacionalidad,
            p_idNacionalidad,
            p_cedPaciente
        );
    END AgregarPacNac;
    ...
    BEGIN
        UPDATE paciente_nacionalidad
        SET idNacionalidad = p_idNacionalidad,
            cedPaciente = p_cedPaciente
        WHERE idPacienteNacionalidad = p_idPacienteNacionalidad;
    END ActualizarPacNac;

    PROCEDURE EliminarPacNac(p_idPacienteNacionalidad NUMBER) AS
    BEGIN
        DELETE FROM paciente_nacionalidad WHERE idPacienteNacionalidad = p_idPacienteNacionalidad;
    END EliminarPacNac;
    
    FUNCTION TraerPacNac(p_idPacienteNacionalidad NUMBER) RETURN paciente_nacionalidad%ROWTYPE AS
        v_PacNac paciente_nacionalidad%ROWTYPE;
    BEGIN
        SELECT *
        INTO v_PacNac
        FROM paciente_nacionalidad
        WHERE idPacienteNacionalidad = p_idPacienteNacionalidad;
...
-- 6. Paquete para el CRUD de tratamiento_dosis 

CREATE OR REPLACE PACKAGE tratamiento_dosis_package AS
    PROCEDURE AgregarTratDosis(
        p_idTratamientoDosis NUMBER,
        p_idTratamiento NUMBER,
        p_idDosis NUMBER
    );

    PROCEDURE ActualizarTratDosis(
        p_idTratamientoDosis NUMBER,
        p_idTratamiento NUMBER,
        p_idDosis NUMBER
    );

    PROCEDURE EliminarTratDosis(p_idTratamientoDosis NUMBER);
    
    FUNCTION TraerTratDosis(p_idTratamientoDosis NUMBER) RETURN tratamiento_dosis%ROWTYPE;
END tratamiento_dosis_package;
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY tratamiento_dosis_package AS
    PROCEDURE AgregarTratDosis(
        p_idTratamientoDosis NUMBER,
        p_idTratamiento NUMBER,
        p_idDosis NUMBER
    ...
    BEGIN
        INSERT INTO tratamiento_dosis(
            idTratamientoDosis,
            idTratamiento,
            idDosis
        ) VALUES (
            p_idTratamientoDosis,
            p_idTratamiento,
            p_idDosis
        );
    END AgregarTratDosis;
    ...
    BEGIN
        UPDATE tratamiento_dosis
        SET idTratamiento = p_idTratamiento,
            idDosis = p_idDosis
        WHERE idTratamientoDosis = p_idTratamientoDosis;
    END ActualizarTratDosis;

    PROCEDURE EliminarTratDosis(p_idTratamientoDosis NUMBER) AS
    BEGIN
        DELETE FROM tratamiento_dosis WHERE idTratamientoDosis = p_idTratamientoDosis;
    END EliminarTratDosis;
    
    FUNCTION TraerTratDosis(p_idTratamientoDosis NUMBER) RETURN tratamiento_dosis%ROWTYPE AS
        v_TratDosis tratamiento_dosis%ROWTYPE;
    BEGIN
        SELECT *
        INTO v_TratDosis
        FROM tratamiento_dosis
        WHERE idTratamientoDosis = p_idTratamientoDosis;
...
-- 7. Paquete para el CRUD de tratamiento_medicamento 

CREATE OR REPLACE PACKAGE tratamiento_medicamento_package AS
    PROCEDURE AgregarTratMed(
        p_idTratamientoMedicamento NUMBER,
        p_idTratamiento NUMBER,
        p_idMedicamento NUMBER
    );

    PROCEDURE ActualizarTratMed(
        p_idTratamientoMedicamento NUMBER,
        p_idTratamiento NUMBER,
        p_idMedicamento NUMBER
    );

    PROCEDURE EliminarTratMed(p_idTratamientoMedicamento NUMBER);
    
    FUNCTION TraerTratMed(p_idTratamientoMedicamento NUMBER) RETURN tratamiento_medicamento%ROWTYPE;
END tratamiento_medicamento_package;
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY tratamiento_medicamento_package AS
    PROCEDURE AgregarTratMed(
        p_idTratamientoMedicamento NUMBER,
        p_idTratamiento NUMBER,
        p_idMedicamento NUMBER
    ...
    END ActualizarTratMed;

    PROCEDURE EliminarTratMed(p_idTratamientoMedicamento NUMBER) AS
    BEGIN
        DELETE FROM tratamiento_medicamento WHERE idTratamientoMedicamento = p_idTratamientoMedicamento;
    END EliminarTratMed;
    
    FUNCTION TraerTratMed(p_idTratamientoMedicamento NUMBER) RETURN tratamiento_medicamento%ROWTYPE AS
        v_TratMed tratamiento_medicamento%ROWTYPE;
    BEGIN
        SELECT *
        INTO v_TratMed
...
-- 8. Paquete para el CRUD de expediente_tratamiento 

CREATE OR REPLACE PACKAGE expediente_tratamiento_package AS
    PROCEDURE AgregarExpTrat(
        p_idExpTrat NUMBER,
        p_idTratamiento NUMBER,
        p_idExpediente NUMBER
    );

    PROCEDURE ActualizarExpTrat(
        p_idExpTrat NUMBER,
        p_idTratamiento NUMBER,
        p_idExpediente NUMBER
    );

    PROCEDURE EliminarExpTrat(p_idExpTrat NUMBER);
    
    FUNCTION TraerExpTrat(p_idExpTrat NUMBER) RETURN expediente_tratamiento%ROWTYPE;
END expediente_tratamiento_package;
--------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY expediente_tratamiento_package AS
    PROCEDURE AgregarExpTrat(
        p_idExpTrat NUMBER,
        p_idTratamiento NUMBER,
        p_idExpediente NUMBER
    ...
    BEGIN
        INSERT INTO expediente_tratamiento(
            idExpTrat,
            idTratamiento,
            idExpediente
        ) VALUES (
    ...
    BEGIN
        UPDATE expediente_tratamiento
        SET idTratamiento = p_idTratamiento,
            idExpediente = p_idExpediente
        WHERE idExpTrat = p_idExpTrat;
    END ActualizarExpTrat;

    PROCEDURE EliminarExpTrat(p_idExpTrat NUMBER) AS
    BEGIN
        DELETE FROM expediente_tratamiento WHERE idExpTrat = p_idExpTrat;
    END EliminarExpTrat;
    
    FUNCTION TraerExpTrat(p_idExpTrat NUMBER) RETURN expediente_tratamiento%ROWTYPE AS
        v_ExpTrat expediente_tratamiento%ROWTYPE;
    BEGIN
        SELECT *
        INTO v_ExpTrat
        FROM expediente_tratamiento
        WHERE idExpTrat = p_idExpTrat;
